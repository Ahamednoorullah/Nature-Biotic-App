...
export default function CompanyReceipts() {
  ...
  const canCreate =
    !!createStoreId && !!receiptNo && !!method && amountReceived > 0;
    ...
    () =>
      [...createdReceipts, ...receipts].filter((r) => {
        ...
        if (dateFilter !== "all") {
          ...
          const now = new Date();
          if (dateFilter === "week")
            md = (now.getTime() - d.getTime()) / 86400000 <= 7;
          else if (dateFilter === "month")
            md = (now.getTime() - d.getTime()) / 86400000 <= 30;
          else if (dateFilter === "quarter")
            md = (now.getTime() - d.getTime()) / 86400000 <= 90;
        }
        return ms && mt && md;
      }),
        ...
        <div className="flex flex-col lg:flex-row gap-3">
          <div className="flex-1 max-w-md">
            <Input
              value={search}
              onChange={setSearch}
              placeholder="Search by receipt no, store, invoice..."
              icon="search"
            />
          </div>
