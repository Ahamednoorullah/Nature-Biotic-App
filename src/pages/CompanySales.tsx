...
function computeAdded(r: Omit<AddedRow, 'key' | 'taxAmount' | 'rowTotal'>): AddedRow {
  ...
  const taxAmount = Math.round(afterDiscount * (r.taxPercent / 100) * 100) / 100;
  const rowTotal = Math.round((afterDiscount + taxAmount) * 100) / 100;
  return { ...r, key: Math.random().toString(36).slice(2), taxAmount, rowTotal };
}
...
export default function CompanySales() {
  ...
  return (
    <div>
      ...
      <Card className="p-4 mb-5">
        <div className="flex flex-col lg:flex-row gap-3">
          <div className="flex-1 max-w-md">
            <Input value={search} onChange={setSearch} placeholder="Search by invoice, store, product..." icon="search" />
          </div>
      ...
      {filtered.length === 0 ? (
        <Card className="p-0">
          <EmptyState icon="point_of_sale" title="No sales found" description="Adjust filters or create a new sale." />
        </Card>
      ) : (
            ...
            <table className="w-full text-sm min-w-[1200px] border-collapse">
              <thead>
                <tr className="bg-slate-100 text-slate-600 text-xs uppercase tracking-wider border-b-2 border-slate-200">
                  ...
                  <th className="text-left font-semibold px-3 py-3 border-r border-slate-200">Product</th>
                  <th className="text-left font-semibold px-3 py-3 border-r border-slate-200">Pack Size</th>
                  <th className="text-right font-semibold px-3 py-3 border-r border-slate-200">Quantity</th>
                  <th className="text-right font-semibold px-3 py-3 border-r border-slate-200">Rate</th>
                  <th className="text-right font-semibold px-3 py-3 border-r border-slate-200">Tax Amount</th>
              ...
              <tbody>
                {filtered.map((s, i) => (
                  <tr key={s.id} className={`border-b border-slate-100 ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/60'} hover:bg-brand-50/40 transition-base`}>
                    ...
                    <td className="px-3 py-3 text-slate-600 border-r border-slate-100">{s.product}</td>
                    <td className="px-3 py-3 text-slate-600 border-r border-slate-100">{s.packSize}</td>
                    <td className="px-3 py-3 text-right tabular-nums text-slate-600 border-r border-slate-100">{s.quantity}</td>
                    <td className="px-3 py-3 text-right tabular-nums text-slate-600 border-r border-slate-100">{formatCurrency(s.rate)}</td>
                    <td className="px-3 py-3 text-right tabular-nums text-slate-600 border-r border-slate-100">{formatCurrency(s.taxAmount)}</td>
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-black/40 p-0">
          ...
          <div className="relative bg-white w-full sm:max-w-[1210px] shadow-elevated flex flex-col h-full sm:h-[90vh] animate-scale-in">
            ...
            <div className="flex items-start justify-between px-5 sm:px-8 py-4 border-b border-slate-100 shrink-0">
              <div>
                <h3 className="text-lg font-bold text-slate-800 tracking-tight">Create Store Sale</h3>
            ...
            <div className="flex-1 overflow-y-auto px-5 sm:px-8 py-6 space-y-7">
                  ...
                  {entryProduct && (
                    <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3 pt-4 border-t border-slate-200">
                      <div className="col-span-2 sm:col-span-1 flex items-center gap-2">
                        <div className={`w-10 h-10 rounded-lg bg-${entryProduct.imageColor}-100 flex items-center justify-center shrink-0`}>
                          <Icon name="image" size={20} className={`text-${entryProduct.imageColor}-600`} />
                        </div>
              ...
              <section className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                ...
                <div className="rounded-2xl bg-slate-50 border border-slate-200 p-5 lg:self-start">
                  <h5 className="text-sm font-bold text-slate-800 mb-3">Invoice Summary</h5>
...
function SummaryRow({ label, value, muted = false }: { label: string; value: string; muted?: boolean }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <span className={`tabular-nums ${muted ? 'text-slate-500 text-xs' : 'font-semibold text-slate-700'}`}>{value}</span>
    </div>
  );
}
